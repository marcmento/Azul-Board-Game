...
ADVG::ADVG(std::string p1, std::string p2, bool random_choice, std::string seed)
{
...
ADVG::ADVG(Player* p1, Player* p2, BST* factory1, BST* factory2, BST* factory3,
...
void ADVG::save(std::string file)
{
    std::ofstream savefile;
    savefile.open(file);
    savefile << "#NewFormat_ADVG\n";
    savefile << seed << "\n";
...
// Prints player info
void ADVG::printplayer(Player *player)
{
    std::cout << std::endl
...
void ADVG::move(Player *player, std::string move)
{
...
void ADVG::roundend()
{
    // Round is increased
    ++roundCount;
    std::cout << "=== Manual Move for " << player1->getName() << " ====" << std::endl;
    manual(player1);
    ...
    // Same as above
    std::cout << "=== Manual Move for " << player2->getName() << " ====" << std::endl;
    manual(player2);
    // Scoring for broken line
...
bool ADVG::validText(std::string playermove)
{
...
bool ADVG::validMove(Player *player, std::string playermove)
{
...
void ADVG::manual(Player* player){
    for(int i = 0; i<ROW_COL_SIZE; i++){
        if(player->getMosaic()->returnIfStorageIsFull(i+1) == true){
            player->getMosaic()->print();
            bool valid = false;
            int col;
            while(valid == false){
                std::cout << std::endl << "Please enter the column you want to place the tile from row " << i + 1<< " in";
                std::cout << std::endl << "> ";
                std::string move;
                ...
                if(move == "1" || move == "2" || move == "3" || move == "4" || move == "5"){
                    ...
                    savegame->add(move);
                    valid = true;                    
                } else if(move.substr(0,4) == "save" || move.substr(0,4) == "Save"){
                    std::cout << std::endl << "** Error: Please move all tiles before saving **";
                } else {
                    std::cout << std::endl << "** Error: Please enter a valid input (Row cannot contain same tile) **";
                }
            }
            if(col == 6){
                int count = 0;
                for(int j = 0; j < ROW_COL_SIZE; j++){
                    if(player->getMosaic()->getStorageTile(i,j)->isColoured() == true){
                        player->getMosaic()->addBroken(player->getMosaic()->getStorageTile(i,j), tilebag, boxlid, random_choice);
                        count++;
                    }
                }
                ...
                this->tilebag = player->getMosaic()->manualMove(i, col-1, tilebag, boxlid, random_choice);
